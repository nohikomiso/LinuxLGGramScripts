...
if [[ $TURN_ON == 1 ]]; then
    ...
    if [ $USE_ACPI_CALL -eq 1 ]; then
        echo "\_SB.PCI0.LPCB.H_EC.ECWX $EC_FNLOCK_ADDR 0x01" | sudo tee $ACPI_CALL > /dev/null
    else
    ...
    if [ $USE_ACPI_CALL -eq 1 ]; then
        echo "\_SB.PCI0.LPCB.H_EC.ECWX $EC_FNLOCK_ADDR 0x00" | sudo tee $ACPI_CALL > /dev/null
    else
